...
public class MessageUnreactEvent extends IncomingEvent {

    public MessageUnreactEvent() {
    ...
    public MessageUnreactEvent(RJA rja, String message, String channel, String user, String emoji) {
        super(rja, "MessageUnreact");
    ...
    public IncomingEvent handle(RJA rja, JsonObject object) {
        ...
        if(old.getReaction(emojiId) != null) {
            if(!old.getReaction(emojiId).contains(user)) {
                return null;  // User has never reacted with this emoji, so this event is invalid
            }
        }
        ...
        return new MessageUnreactEvent(rja, message, channel, user, emoji);
